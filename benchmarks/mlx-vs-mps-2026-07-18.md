...
**速度では[MLX版](https://github.com/ssmall256/mlx-audio-separator)が優秀だった。** Apple M4 Proで実製品と同じ4モデル・5推論をつないだ34.44秒の分離チェーンは、最適化済みMPS版の34.470秒からMLX版の27.969秒へ **18.86%短縮（1.232倍）** した。process peak RSSの中央値も約2.99 GiBから約1.86 GiBへ下がった。229.18秒の各Roformerを別processで個別に常駐させたwarm条件でも、MLX版はMPS regional compileより**10.51%短かった（1.117倍）**。
...
| Python | 3.12.3 |
| MPS側 | `audio-separator` 0.41.1、PyTorch 2.13.0、主比較revision `046447c`、229秒warm追加revision `b99bb42` |
...
`LifeLight.wav`全体を使い、KimとKaraokeのmodel単体を測った。どちらにも同じ元音源を直接入力しているため、2モデルの和は製品chainではない。`separate()`の計測にはWAV decode、model処理、WAV書き出しを含み、model loadは別timerとした。

まず、従来のcold比較は各model・各条件をfresh processで1回実行した値である。MPS compileはmodelごとに別のempty Inductor cacheから開始し、MLXは事前変換済みweightを使った。
...
各modelを別のfresh processでloadし、同じ229.18秒入力を1回フルにwarm-upした後、**同一の`Separator` / model instanceで3回**実行して中央値を取った。MPS compileは各modelのprocess開始前に固有のempty Inductor cacheを用意し、warm-upでcompileを完了させた。MLX側も遅延初期化やfile cacheの条件をそろえるため同じく1回warm-upした。表の合計と比較率にはloadを含めない。

| 条件 | warm-up（Kim / Karaoke） | Kimの3回 → 中央値 | Karaokeの3回 → 中央値 | 2モデル中央値和 |
|---|---:|---:|---:|---:|
| MPS eager | 58.676 / 58.314秒 | 57.947 / 57.858 / 57.986 → **57.947秒** | 57.962 / 58.000 / 57.896 → **57.962秒** | **115.910秒** |
| MPS regional compile | 46.303 / 46.376秒 | 42.802 / 42.566 / 42.530 → **42.566秒** | 42.667 / 42.788 / 44.104 → **42.788秒** | **85.354秒** |
| MLX（変換済みweight） | 38.041 / 37.854秒 | 38.788 / 38.583 / 38.342 → **38.583秒** | 37.803 / 37.802 / 38.544 → **37.803秒** | **76.386秒** |

warm定常ではMPS regional compileがeagerを**26.36%短縮（1.358倍）**した。同じ追加計測内でもMPS compileのwarm-up和92.678秒から定常中央値和85.354秒へ7.90%短くなった。従来のfresh-process初回値93.169秒と比べるとwarmは8.39%短いが、別sessionの単発値との比較なので、この差全てをcompile初期化だけのコストとはみなさない。それでもMLXはwarm compileより**8.968秒、10.51%短く、1.117倍**だった。fresh-process初回比較で見えた17.95%差はwarm同士では10.51%まで縮まるが、優劣は逆転しなかった。

このwarm値は、各Roformerを別processで個別に常駐させ、**同じ入力長・同じshapeを同じmodel instanceへ繰り返した場合**の定常性能である。現在の製品pipelineは`run()`呼び出しごとに`Separator`を新規作成し、`use_torch_compile`も渡していない。そのため「現製品の2曲目以降が85.354秒になる」という意味ではなく、常駐化とcompile opt-inを実装した場合の上限寄りの参考値である。任意の曲長、4モデルの製品chain、endpoint E2Eへ一般化するには別計測が必要になる。

追加計測はバッテリー通常電力モード（`powermode 0`）で、Kimをcompile → MLX → eager、Karaokeをeager → MLX → compileの順に実行した。バッテリー残量は58%から23%へ低下したが、終了時にmacOSのthermal / performance warningはなかった。最後のKaraoke compile 3回目は中央値より3.08%遅いため、全実測値を上表とaggregate JSONへ残した。
...
差が特に大きいのはMLX版のDeEcho reverb（38.791%）とDemucs drums（25.871%）だった。ただしdrumsはこの区間の基準RMSが約0.0086（約-41 dBFS）と小さく、relative L2の割合が増幅されて見える。同じ低い分母に対してMPS版は0.213%なので、backend間の相対比較自体は有効である。MPS対MLXの直接比較もほぼ同じ値になり、MPS版がCPU fp32へ近く、MLX版の実装差が波形へ現れていることを確認した。
...
- 当初のMPS / MLX速度計測: benchmark JSONが記録したrevisionは`046447c7f3553cc1eef8a3b391f331391b38393f`。この時点のrunnerはdirty状態を記録しないため、revision以外は遡って断定しない。
- 229.18秒warm追加計測: revision `b99bb421f419abedaee8179c0e993997f290edee`。runnerへMPSの実効native fp16 / compile状態と`TORCHINDUCTOR_CACHE_DIR`の記録だけを追加し、推論経路は変更していない。6つのraw JSONは全て`tracked_dirty=false`、`untracked_file_count=1`を記録した。MPS compileの2結果は`is_native_mps_fp16=true`かつ`is_torch_compiled=true`だった。
...
229.18秒のwarm定常計測は、次のfunctionを下記の順で呼んだ。compile条件ではmodelごとに新しい空directoryを使う。`BENCH_ROOT`自体も新しいdirectoryから開始する。
...
run_229s_warm() {
  ...
  env_args=("HOME=$BENCH_HOME")
  if [[ "$compile_flag" == "--torch-compile" ]]; then
    cache_dir="$BENCH_ROOT/cache-$label"
    mkdir "$cache_dir"
    env_args+=("TORCHINDUCTOR_CACHE_DIR=$cache_dir")
  fi

  env "${env_args[@]}" "$BENCH_PYTHON" tools/benchmark_apple_backends.py \
    "$SOURCE_DIR/LifeLight.wav" \
    --backend "$backend" --mode models --model "$model" \
    --model-dir "$MODEL_DIR" \
    --output-dir "$BENCH_ROOT/output/$label" \
    --json-output "$BENCH_ROOT/results/long-warm-$label.json" \
    --warmup 1 --repeats 3 --seed 0 "$compile_flag" \
    --mlx-speed-mode default \
    --mlx-cache-clear-policy aggressive \
    --mlx-write-workers 1 \
    --mlx-demucs-batch-size 1 \
...
run_229s_warm mps kim mps-compile-kim --torch-compile
run_229s_warm mlx kim mlx-kim --no-torch-compile
run_229s_warm mps kim mps-eager-kim --no-torch-compile
run_229s_warm mps karaoke mps-eager-karaoke --no-torch-compile
run_229s_warm mlx karaoke mlx-karaoke --no-torch-compile
run_229s_warm mps karaoke mps-compile-karaoke --torch-compile
```
...
```text
pytest tests/unit
233 passed, 4 skipped, 4 warnings
```
...
- 229.18秒のRoformer 2モデルをMPS eager / regional compile / MLXそれぞれでwarm-up 1回 + 3計測（計18 timed run）し、6 processすべての最終出力validation成功を確認
- 229.18秒warm計測のMPS compile 2結果で、native fp16とregional compileの実効値がともに`true`であることを確認
