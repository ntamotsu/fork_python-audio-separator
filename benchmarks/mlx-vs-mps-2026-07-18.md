...
MPS cold compileは、modelごとに新しいprocessを起動した**初回実行**の値である。製品の常駐FastAPI processで2曲目以降も同じcompile graphを再利用できるなら、compile初期化分がなくなりMPSはこの値より短くなるが、229.18秒入力のwarm定常状態は今回測っていない。したがって「MLXがcold compileより17.95%短縮」は初回同士の比較であり、定常状態の短縮率ではない。
